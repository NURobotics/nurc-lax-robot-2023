...
def detect_bboxes(yolo_net, layerNames, confid, tracking=False):
    '''Carries out the heavy lifting of the object detection model. Takes
    in the YOLO model, detects potential matches for the "ball", and uses
    non-maximum suppression (NMS) to remove redundant data.

    Returns: boundingboxes, a list of bounding box objects
    '''
...
def process_frame(vs, H, W, ct, yolo_net, layerNames, confid, tracking=False):
    '''Takes a source of video input, captures + resizes a frame, locates the objects of interest,
    and carries out centroid tracking.

    Returns: the modified frame, as well as the bounding boxes that have been edited
    by centroid tracking code
    '''
    ...
    boundingboxes = detect_bboxes(yolo_net, layerNames, confid, tracking=tracking)
...
def detection_and_tracking(vid_input, yolo_net, layerNames, confid, threshold, disappeared, tracking=False):
    ...
    while True:
        ...
        frame, boundingboxes = process_frame(vs, H, W, ct, yolo_net, layerNames, confid, tracking=tracking)
        if frame is None: #error handling
            break
